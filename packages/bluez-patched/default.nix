...
pkgs.bluez.overrideAttrs (oldAttrs: {
  ...
  #
  # BlueZ 5.84 uses direct uint16_t assignment for metadata context
  # 0x0001 (Unspecified) -> 0x0002 (Conversational)
  doCheck = false;
  ...
  postPatch = (oldAttrs.postPatch or "") + ''
    ...
    substituteInPlace src/shared/bap.c \
      --replace-fail 'cpu_to_le16(0x0001); /* Context = Unspecified */' \
